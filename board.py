...
## Status bar object containing all the attributes of the top status 
# Describes bomberman's remaining lives
# Describes the time left in a level
# Describles the player's score
...
## Gameplay controller of the bomberman game
class Board(QtGui.QFrame):

    ## Signal sent to the main controller when the game is paused
    pauseGameSignal = QtCore.pyqtSignal()

    ## Signal sent to the main controller when the game is over
    gameOverSignal = QtCore.pyqtSignal()

    ## Signal sent to to the main controller when the time has reset, e.g. when a new level starts
    resetTimerSignal = QtCore.pyqtSignal()

    ## Signal sent to the main controller when update score in the database is required
    updateScoreInDbSignal = QtCore.pyqtSignal(int)

    ## Constructor taking a level object as argument
    ...
    ## Initialize a status bar and sets it in a fix position
    ...
    ## Initialize a game board inside the level object
    def initBoard(self):
    ...
    ## Start the initial level gameplay
    def initLevel(self):
